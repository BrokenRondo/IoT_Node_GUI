#pragma once
#ifndef MERKLETREEBIN
#define MERKLETREEBIN
#include <cstdio>
#include <math.h>
#include <stdexcept>
#include <string.h>
...
inline void printVect(vector<int> v){
...
// leafCount => nodeCount
inline int leafCountToNodeCount(const int leftCount){
  return leftCount*2 -1;
}

// nodeCount => leafCount
inline int nodeCountToLeafCount(const int nodeCount){
  return (nodeCount +1 )/2;
}

// isn't leaf
inline bool isInteriorNode(int idx,int nodeCount){
...
// leaves => tree
template <typename T>
inline vector<T> buildFromLeaves(const vector<T>& leaves){
...
// tree => leaves
template <typename T>
inline vector<T> leaves(const vector<T>& tree){
...
// get root
template <typename T>
inline T root(vector<T> tree){
  return tree[0];
}

template <typename T>
inline vector<int> levelOrder(const vector<T>& tree){
...
template <typename T>
inline int guardRange(const vector<T>& tree,int idx,int cal_idx){
...
template <typename T>
inline int getParent(const vector<T>& tree,int idx){
  int parent_idx = floor((idx - 1) / 2);
  return guardRange<T>(tree,idx,parent_idx);
}

template <typename T>
inline int getLeft(const vector<T>& tree,int idx){
  int left_idx = 2*idx +1;
  return guardRange<T>(tree,idx,left_idx);
}

template <typename T>
inline int getRight(const vector<T>& tree,int idx){
...
// pass by ref *******************************************************
template <typename T>
inline void climb(vector<T>& tree,int idx,void (*fn)(T&, int)){
  while(idx > 0){
    //printf("sadada\n" );
...
// get index of leaf
template <typename T>
inline int findLeaf(const vector<T>& tree,const T& leaf){
  int leafCount = nodeCountToLeafCount(tree.size());
